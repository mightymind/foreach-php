<?
/*
	Класс SQLite для фреймворка ForEach
...
class SQLite {
	...
	public function _init($base, $mode = 0666, $auto = true) {
		$this->db_file=$base;
	...
	function select($table,$param='') {
		return $this->query("SELECT * FROM $table $param");
	}
	
	function insert($table,$value) {
		return $this->query("INSERT INTO $table VALUES($value)");
	}
	
	function delete($table,$param='') {
		return $this->query("DELETE FROM $table $param");
	}
	
	function update($table,$newvalue,$param='') {
		return $this->query("UPDATE $table SET $newvalue $param");
	}
